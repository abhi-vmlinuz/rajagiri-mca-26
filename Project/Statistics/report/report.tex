...
cricket tournaments, and every edition produces a rich, publicly available
...
\subsection*{1.1 Dataset and Variables}
...
\subsection*{2.1 Central Tendency}
For \emph{target runs} the mean is 160.54, the median 163,
and the mode 166 runs. The mean and median are close, indicating that
first-innings targets are broadly symmetric about a typical value near the
160--165 run mark. For \emph{victory margins} the mean is 17.08 and the
median only 8; a mean well above the median signals a
distribution dragged upward by a modest number of very large margins.

\subsection*{2.2 Dispersion}
The standard deviation of targets is 32.41 runs and that of margins
21.95 runs. Because the two variables differ in scale, the coefficient
of variation is more informative: margins have a coefficient of
variation 1.285 --- more than six times that of targets
(0.202). In practical terms, targets are tightly clustered around 160,
whereas match margins are highly volatile.

The quartiles confirm the asymmetry. For targets the interquartile range spans
142--182 (IQR 40); for margins the IQR spans
6--19 (IQR 13), with a maximum of
146. The skewness of margins is 2.81, strongly positive,
reflecting abundant narrow wins and rare blow-outs. Targets are only mildly
negatively skewed (-0.46).
...
compressed margin box with its whisker to 146 illustrates the strong
right skew described in Section 2.2.}
\end{figure}

\section*{3. Probability Modelling}
\subsection*{3.1 The Binomial Model for First-Innings Advantage}
...
279 matches in which the toss winner chose to bat first,
the same side won 128, giving
$$\hat{p}_{\text{bat}} = \frac{128}{279}
= 0.459.$$
In contrast, of the 407 matches in which the toss winner
chose to field, the chasing side won 226, so that
$$\hat{p}_{\text{chase}} = 0.555.$$

Assuming a fair coin, $p_0 = 0.5$ for either strategy. The exact two-sided
Binomial test of $H_0: p = 0.5$ against $H_1: p \ne 0.5$ yields a
$p$-value of 0.188 for batting first --- not
significant at $\alpha = 0.05$ --- but 0.029
for chasing, which \emph{is} significant. The 95\% confidence intervals are
$$\text{bat first: }\ [0.399,\ 0.519]
\qquad
\text{chasing: }\ [0.506,\ 0.604].$$
...
\subsection*{3.2 A Probability Mass Function for Wicket Margins}
When a match is won by chasing (n = 370), the margin is recorded as the
...
outcome is $X = 6$ with $P(X = 6) = 0.205$. The
...
370 chases won by the fielding side.}
\end{figure}

\subsection*{3.3 Range-Based Probabilities}
...
P(T < 120) &= 0.093.
\end{align*}
Nearly 0.233*100\% of all targets fall in the 140--160 band
and a further 0.389*100\% in the 161--190 band, so a team
setting a target above 190 enters territory reached in only about
0.087*100\% of matches. For victory margins, P(margin $>$ 50) =
0.074 and P(margin $\ge$ 100) = 0.012,
...
$$\hat{y} = 153.83 + 1.15\,x,$$
implying an average increase of 1.15 runs per season. The fitted
model explains 0.405*100\% of the variance in seasonal mean targets
($R^2 = 0.405$), and the slope is significant at the 5\% level
($p = 0.035$).
...
upward by roughly 1.2 runs per season --- a statistically
...
blow-outs beyond 50 are rare (about 0.074*100\% of decided
matches).

On the probabilistic side, the Binomial model reveals a genuine first-innings
advantage for the chasing side ($\hat{p} = 0.555$,
$p = 0.029$), a result consistent with the fielding
advantage under floodlights. Team dominance is real but measured: Chennai Super
Kings won 90 of their 146 matches
($\hat{p} = 0.616$, $p = 0.006$), a rate the
Binomial test confirms against a fair-coin null, while Mumbai Indians
($\hat{p} = 0.571$) fall just short of conventional significance.
...
\vspace{1em}
\noindent\emph{Software.} All computations were performed in Python
(\texttt{pandas}, \texttt{scipy}) and all figures were produced in R
(\texttt{ggplot2}); this report was typeset in \LaTeX. The analysis is
fully reproducible from the scripts in the repository.
