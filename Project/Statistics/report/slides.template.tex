...
Mean MEAN\_TM, median MED\_TM, mode MODE\_TM\\
SD SD\_TM \quad CV CV\_TM\\[0.4em]
\textbf{Victory margin} (n = 685)\\
Mean MEAN\_MM, median MED\_MM\\
SD SD\_MM \quad CV CV\_MM\\[0.4em]
Margins are strongly right-skewed (skew SKEW\_MM):\\
most games are close, blow-outs are rare --- yet the
maximum reaches MAX\_MM runs.
...
\end{center}
\begin{itemize}
\item Targets: tight cluster around 160--165 (IQR IQR\_TM)
\item Margins: compressed box, long whisker to MAX\_MM $\Rightarrow$ right skew
\end{itemize}
\end{frame}
...
\midrule
Bat first & K\_BAT / N\_BAT & 0.459 & PV\_BAT\\
Chase & K\_FIELD / N\_FIELD & \textbf{0.555} & \textbf{PV\_FIELD}\\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.8em}
Chasing estimate is \textbf{significantly} above 0.5 ($p=$ PV\_FIELD):\\
